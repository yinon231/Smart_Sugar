// Fetch the meal data from meal_data.json file
fetch('meal_data.json')
  ...
  .then(data => {
    ...
    // The rest of the JavaScript code starts here
    ...
    function saveMeal(meal) {
      const mealFoods = Array.from(foodList[meal].children).map(foodItem => {
        return {
          name: foodItem.textContent.split(" (")[0],
          calories: parseFloat(foodItem.getAttribute("data-calories")),
          sugar: parseFloat(foodItem.getAttribute("data-sugar"))
        };
      });

      // Replace the following code with your database saving logic
      console.log("Saving meal to the database:");
